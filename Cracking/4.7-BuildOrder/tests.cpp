...
template<typename T>
bool CheckRes(std::vector<T> const& build_order,
              std::vector<T> const& projects,
              std::vector<std::pair<T, T>> const& deps) {
  std::unordered_map<T, size_t> position{};
  size_t pos{0};
  if (!std::is_permutation(begin(projects), end(projects), begin(build_order))) {
    return false;
  }
  if (!std::is_permutation(begin(build_order), end(build_order), begin(projects))) {
    return false;
  }
...
TEST(BuildOrder, BuildOrderExample) {
  Solution uut;
    ...
    std::vector<std::pair<std::string, std::string>> dependencies{{"a", "d"},
                                                                {"f", "b"},
                                                                {"b", "d"},
                                                                {"f", "a"},
                                                                {"d", "c"},
...
TEST(BuildOrder, BuildOrderExample2) {
  ...
  std::vector<std::string> projects{{"a", "b", "c", "d", "e", "f", "g"}};
    std::vector<std::pair<std::string, std::string>> dependencies{{"a", "e"},
                                                               {"b", "e"},
                                                               {"b", "a"},
                                                               {"f", "a"},
                                                               {"c", "a"},
