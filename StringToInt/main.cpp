 ...
 * 3. Read in next the characters until the next non-digit charcter or the
 ...
 *    integers less than -231 should be clamped to -2^31, and integers greater than
 ...
 * Input: s = "42"
 * Output: 42
 * Explanation: The underlined characters are what is read in, the caret is
 ...
 * 
 * https://leetcode.com/submissions/detail/527628875/
 * Runtime: 4 ms, faster than 63.32% of C++ online submissions for String to Integer (atoi).
 * Memory Usage: 7 MB, less than 48.58% of C++ online submissions for String to Integer (atoi).
*/
...
class Solution {
    const int int_min_10 = std::numeric_limits<int>::min() / 10;
    const int int_max_10 = std::numeric_limits<int>::max() / 10;
    ...
    int myAtoi(std::string_view str) {
        ...
        for (; cnt < str.size(); ++cnt) {
            ...
            if (negative == true) {
                ...
                if (res < num + std::numeric_limits<int>::min()) {
                    res = std::numeric_limits<int>::min();
                    break;
                } else {
                    res -= num;
                ...
                if (res > std::numeric_limits<int>::max() - num) {
                    res = std::numeric_limits<int>::max();
                    break;
                } else {
                    res += num;
...
int main(void) {
    auto obj = Solution();
    std::cout << obj.myAtoi("-91283472332") << '\n';  // -2147483648
    std::cout << obj.myAtoi("-2147483647") << '\n';  // -2147483647
    std::cout << obj.myAtoi("-2147483648") << '\n';  // -2147483648
    std::cout << obj.myAtoi("-2147483649") << '\n';  // -2147483648
    std::cout << obj.myAtoi("2147483647") << '\n';  // -2147483647
    std::cout << obj.myAtoi("2147483648") << '\n';  // -2147483647
    std::cout << obj.myAtoi("2147483646") << '\n';  // -2147483646
    std::cout << obj.myAtoi("42") << '\n';  // 42
    std::cout << obj.myAtoi("-42") << '\n';  // -42
    std::cout << obj.myAtoi("4193 with words") << '\n';  // 4193
    std::cout << obj.myAtoi("words and 987") << '\n';  // 0
    return 0;
}
