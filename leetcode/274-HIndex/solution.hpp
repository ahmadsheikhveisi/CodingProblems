...
class Solution {
 public:
    int hIndex([[maybe_unused]] std::vector<int>& citations) {
        std::vector<int> count(citations.size() + 1, 0);
        for (auto element : citations) {
            ++count[element < static_cast<int>(citations.size()) ? element : citations.size()];
        }
        for (int cnt{static_cast<int>(count.size()) - 2}; cnt >= 0; --cnt) {
            count[cnt] += count[cnt + 1];
        }
        int h_index{0};
        for (int cnt{static_cast<int>(count.size()) - 1}; cnt >= 0 ; --cnt) {
            if (count[cnt] >= cnt) {
                return cnt;
            }
        }
