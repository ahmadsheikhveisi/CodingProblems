...
* 
*/
#ifndef SNAKES_AND_LADDERS_909
#define SNAKES_AND_LADDERS_909
...
class Solution {
 public:
    int snakesAndLadders([[maybe_unused]] std::vector<std::vector<int>>& board) {
        auto n = static_cast<int>(board.size());
        auto board_size = n*n;
        auto visit = std::vector<bool>(board_size, false);
        std::queue<int> st{};
        int dice_roll{0};
        if (board[n - 1][0] != -1) {
            st.push(board[n - 1][0]);
            visit[board[n - 1][0]] = true;
        } else {
            st.push(1);
            visit[0] = true;
        }
        ...
        while (!st.empty()) {
            ...
            while (q_size > 0) {
                ...
                for (int next{curr + 1}; next <= std::min(board_size, curr + 6); ++next) {
                    auto row = n - 1 - ((next - 1)  / n);
                    auto column = ((((next - 1)  / n) & 0x01) == 0) ? ((next - 1) % n) : (n - 1 - ((next - 1) % n));
                    auto temp = next;
                    if (board[row][column] != -1) {
                        // there is ladder or snake
